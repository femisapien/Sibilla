...
snew='01-Jan-2024';%<<---
enew='28-Jan-2026';%<<---
a_upt=0;  %get bloomberg data for assets
s_upt=0;  %get bloomberg data for stocks
...
out=INDICATORS(a_upt,join,indicators,ipath,sectype,sold,eold,snew,enew,listname,updq,type, ...
    ma_p, ma_consecutive, gap_pct, gap_lookback, db_path, f1, f2);

clear type fdel ipath join listname out indicators sectype   updq 
clc
...
out=INDICATORS(s_upt,join,indicators,ipath,sectype,sold,eold,snew,enew,listname,updq,type, ...
    ma_p, ma_consecutive, gap_pct, gap_lookback, db_path, f1, f2);


clear type fdel ipath join listname out indicators sectype updq sold eold snew ma_p ma_consecutive gap_pct gap_lookback db_path f1 f2
clc
...
out=xMRC2(a_upt,join,reports,ipath,sectype,sold,eold,snew,enew,listname,updq,type)

status = WriteOutImproved(db_path, 'ASSETS', enew, 'AC')

clear type fdel ipath join listname out indicators sectype  update updq 
clc


%% MASTER
join=0;  %complete database. Still can be killed below with updq
reports=1;%run complete set of reports
...
out=xMRC2(s_upt,join,reports,ipath,sectype,sold,eold,snew,enew,listname,updq,type)
